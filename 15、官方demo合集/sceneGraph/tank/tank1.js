...
export class Tank extends Object3D {
    ...
    init() {
        ...
        this.tankPosition = new Vector3();
        this.tankTarget = new Vector3();
    ...
    createCurve(params) {
        ...
        const tmpPoints = controlPoints.map((p, ndx) => {
            p0.set(...p);
            p1.set(...controlPoints[(ndx + 1) % controlPoints.length]);
            return [
                (new Vector3()).copy(p0),
                // (new Vector3()).lerpVectors(p0, p1, 0.1),
                // (new Vector3()).lerpVectors(p0, p1, 0.9),
            ];
        }).flat();
    ...
    moveTank(time) {
        ...
        console.log('tankPosition1',JSON.stringify(tankPosition));

        tankPosition.applyMatrix4(this.splineObject.matrixWorld);
        console.log('tankPosition2',JSON.stringify(tankPosition));

        this.tank.position.copy(tankPosition);
        // 设置坦克看向的位置，设置坦克的前进方向
        // this.tank.lookAt(tankTarget.x, 0, tankTarget.y);
        tankTarget.applyMatrix4(this.splineObject.matrixWorld);
        this.tank.lookAt(tankTarget.x, tankTarget.y, tankTarget.z);
